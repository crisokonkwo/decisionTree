...
class DecisionTree:
    ...
    def train(self, features, labels, num_leaves=None):
        ...
        while len(leaves) < max_rounds:
            ...
            left_part = X_node[:, feat] <= thresh
            if left_part.sum() == 0 or left_part.sum() == X_node.shape[0]:
                # cannot split
                leaf_to_split.best_split = None
                print("(degenerate split, skipping)")
                continue

            l_indices = leaf_to_split.indices[left_part]
            r_indices = leaf_to_split.indices[~left_part]
